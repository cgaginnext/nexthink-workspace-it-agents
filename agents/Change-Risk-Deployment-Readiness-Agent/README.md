...
Change Risk & Deployment Readiness Agent is a Nexthink Workspace IT Agent designed to support EUC, Endpoint Engineering, Transformation, Operations, Workplace Engineering teams.

It focuses on identifying deployment risks prior to Windows, application, or security rollouts and recommends mitigation strategies.
...
`EUC` `Endpoint Engineering` `Transformation` `Operations` `Workplace Engineering`
...
> This agent is specialized in change risk & deployment readiness agent use cases. Please use Nexthink Assist or a dedicated agent for other topics.
