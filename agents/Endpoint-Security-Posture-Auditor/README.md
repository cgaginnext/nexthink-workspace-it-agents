...
Endpoint Security Posture Auditor is a Nexthink Workspace IT Agent designed to support Security, SecOps, Endpoint Engineering, EUC, Compliance teams.

It focuses on evaluating endpoint security posture, compliance, operational hygiene, and configuration consistency across the digital workplace estate.
...
`Security` `SecOps` `Endpoint Engineering` `EUC` `Compliance`
...
> This agent is specialized in endpoint security posture auditor use cases. Please use Nexthink Assist or a dedicated agent for other topics.
