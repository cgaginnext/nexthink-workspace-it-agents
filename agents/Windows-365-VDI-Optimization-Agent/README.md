...
Windows 365 & VDI Optimization Agent is a Nexthink Workspace IT Agent designed to support VDI, Cloud, Infrastructure, EUC, Workplace Engineering teams.

It focuses on optimizing Citrix, AVD, Horizon, and Windows 365 environments using endpoint and virtualization telemetry.
...
`VDI` `Cloud` `Infrastructure` `EUC` `Workplace Engineering`
...
> This agent is specialized in windows 365 & vdi optimization agent use cases. Please use Nexthink Assist or a dedicated agent for other topics.
...
> Identify the VDI, AVD, Citrix, Horizon, and Windows 365 populations experiencing the highest structural degradation by correlating session latency, login delays, roaming profile issues, profile corruption, storage bottlenecks, GPU saturation, network instability, endpoint constraints, and session disconnect trends.
