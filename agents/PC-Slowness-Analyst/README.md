...
PC Slowness Analyst is a Nexthink Workspace IT Agent designed to support IT Operations, Service Desk, EUC, Endpoint Engineering, DEX, Support teams.

It focuses on helping IT teams investigate, prioritize, and reduce recurring PC slowness by identifying impacted devices, qualifying symptom patterns, detecting quick wins, and surfacing structural endpoint performance issues.

The agent acts as an IT operations and support analyst. It helps operators turn Nexthink telemetry into clear investigation paths, prioritized findings, and practical next actions.

## Interested Teams

`IT Operations` `Service Desk` `EUC` `Endpoint Engineering` `DEX` `Support`
...
> This agent is specialized in pc slowness analyst use cases. Please use Nexthink Assist or a dedicated agent for other topics.
...
> Identify the top 10 slowest PCs to treat first, focusing on recurring issues, quick wins, and structural root causes.

## Advanced Example Prompt

> Analyze recurring PC slowness across the managed workplace device estate over the last 30 days, excluding servers and limiting the scope to devices matching the defined naming pattern. Focus only on user-present working periods when possible, clearly separate recurring degradation from isolated episodes, identify the top 10 devices to treat first, qualify the slowness pattern, correlate endpoint telemetry, highlight quick wins, detect structural patterns, and conclude with a practical action plan for a small operational service desk team.
