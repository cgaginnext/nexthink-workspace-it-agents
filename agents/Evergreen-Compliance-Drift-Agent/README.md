...
Evergreen Compliance & Drift Agent is a Nexthink Workspace IT Agent designed to support Endpoint Engineering, Security, EUC, Compliance, Operations teams.

It focuses on monitoring deviations from enterprise standards across OS versions, policies, applications, and operational consistency.
...
`Endpoint Engineering` `Security` `EUC` `Compliance` `Operations`
...
> This agent is specialized in evergreen compliance & drift agent use cases. Please use Nexthink Assist or a dedicated agent for other topics.
