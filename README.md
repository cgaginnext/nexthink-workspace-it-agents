...
| [Digital Experience Root Cause Analyst](./agents/Digital-Experience-Root-Cause-Analyst/) | `IT Operations` `DEX` `Endpoint Engineering` `Support` | Produces structured, evidence-backed root cause analysis reports for DEX score degradation, correlating endpoint performance, application stability, and change events with a five-tier confidence framework. | *"Why did the DEX score drop for the Finance department last week, and what is the root cause?"* |
| [Printing & PaperCut IT Agent](./agents/Printing-&-PaperCut-IT-Agent/) | `IT Operations` `Service Desk` `EUC` `Support` | Guides L1/L2 support agents through structured, evidence-based triage and remediation of Windows Print Spooler and PaperCut client issues, from factualization to escalation. | *"A user can't print and PaperCut isn't popping up — help me triage this device step by step."* |
...
| **Monthly Patching Intelligence Agent** | Analyze the current monthly Windows KB patching cycle across the managed device estate. Identify total deployment coverage, devices that have not yet received the update, devices where installation failed or is pending restart, and populations with recurring patching failures. Segment results by OS version, device model, location, and business entity. Flag devices with the highest exposure risk based on patch age, criticality, and endpoint health context. Recommend prioritized remediation actions distinguishing quick wins from structural patching hygiene issues. At the start of each analysis, include the date and time and the KB reference for the current month's cumulative update. |
...
| **Digital Experience Root Cause Analyst** | Perform a structured root cause investigation for the devices or cohorts showing the largest DEX score degradation over the defined analysis window. Establish the DEX score baseline and identify which subscore drove the largest delta, then correlate endpoint resource utilization, application stability, boot and login performance, and recent change events within the correlation window. Apply the five-tier confidence framework to classify the primary cause, contributing factors, and any alternative hypotheses considered. Clearly state any data gaps and their impact on confidence. Produce the investigation as structured progress updates followed by the final RCA report. At the start of each analysis, include the explicit analysis window as dates. |
| **Executive Risk & Exposure Intelligence Agent** | Continuously evaluate active security exposure indicators and fleet health signals across the estate, computing or updating the Composite Risk Score using the defined weights and automatically applying override rules for executive devices and combined Critical patterns. Check for named risk patterns before reporting individual indicators, since certain combinations carry a higher severity than their components taken separately. Select the correct executive reporting template based on the trigger condition and intended audience, translating every technical finding into a business consequence before presenting it. When a finding requires action, recommend the matching remediation playbook and respect the cross-playbook sequencing rules. Declare any missing required configuration explicitly rather than producing silently incomplete output. At the start of each analysis, include the date and time. |
| **Nexthink Assist IT Agent Designer** | Review all IT Agent Role & Behavior documents and knowledge source file sets currently in use for platform-constraint violations, capability overreach, scope ambiguity, and retrieval risks in the knowledge source files. Flag any document exceeding the character limits, any design relying on custom fields, custom trends, or external system access, and any scheduled task instruction that references relative time expressions or prior-run state. Classify each finding as blocking, minor, or acceptable, and recommend the specific correction needed. At the start of each review, include the date and time. |
| **Printing & PaperCut IT Agent** | Identify devices across the fleet showing Print Spooler crashes, hangs, or stuck queues, and PaperCut client issues such as authentication failures, quota errors, or client connectivity problems, over the past 7 days. For each affected device, confirm whether the Print Spooler service is currently running, the installed PaperCut client version, and any recent application crashes or system events related to printing, and correlate the onset with recent package installs or updates. Distinguish single-user, single-device, and site-wide or server-side patterns, and prioritize devices where an automated remediation such as a Print Spooler reset is available. Summarize findings with the evidence needed for L2 escalation where automated remediation is not applicable. At the start of each analysis, include the date and time. |
...
| **Monthly Patching Intelligence Agent** | Conduct a comprehensive end-to-end analysis of the current monthly Windows KB patching cycle across the full managed device estate. Begin by identifying the KB reference for this month's cumulative update for each Windows version present in the environment (Windows 10 21H2, 22H2, Windows 11 22H2, 23H2, 24H2, and any others detected). For each OS version, report total devices in scope, devices that have successfully installed the update, devices where installation is pending restart, devices where installation failed, and devices with no installation attempt recorded. Segment all findings by OS version, hardware model, location, organizational unit, and business entity to surface structural patching gaps. Identify devices that have missed two or more consecutive monthly updates and flag them as chronic non-compliant endpoints. For devices with installation failures, correlate with available endpoint signals such as low disk space, unhealthy collectors, stale last-seen dates, pending reboots exceeding 14 days, or known compatibility issues, and classify the most probable failure cause for each population. Cross-reference unpatched devices with endpoint security posture indicators to estimate relative exposure severity. For each identified gap population, recommend a prioritized remediation action specifying the expected operational effort, the team responsible, the follow-up indicator, and whether the action is corrective, preventive, or hygiene-oriented. Conclude with an executive-ready patching scorecard summarizing overall fleet compliance rate, highest-risk populations, top failure drivers, and the recommended immediate actions for the next 72 hours. At the start of the analysis, include the date, time, and the KB references analyzed. |
...
| **Digital Experience Root Cause Analyst** | Conduct a complete ten-step root cause investigation for DEX score degradation across the requested scope. Begin by confirming the scope (single device, cohort, department, or organization) and converting any relative time expression into explicit dates. Retrieve the DEX score baseline (overall, Technology, Endpoint, Applications, Collaboration, Sentiment) for the defined window and identify which subscore drove the largest delta; if Sentiment is the primary driver, stop the technical investigation and report that finding alone. Otherwise, inventory the affected population including OS distribution and Collector version range, excluding devices with insufficient data. Query CPU, memory, disk, and network utilization for the affected population, recording P50 and P95 values and flagging threshold breaches. Query application crash and freeze events, classifying severity as Isolated, Recurring, or Severe, and identify the top offending applications. If the Applications subscore is degraded, analyze web application page load times and transaction durations, flagging incomplete transaction ratios above 20%. Query boot duration and login desktop-visible time P50 and P95, correlating timing with the degradation onset without attributing cause without cross-referencing other signals. Query package installs, uninstalls, and OS updates within the correlation window and classify each as Causal, Contributory, Coincidental, or Candidate — never Causal without before/after metric confirmation. Synthesize all signals using the five-tier confidence framework (Confirmed / Probable / Candidate / Coincidental / Insufficient Evidence) to identify the primary cause, contributing factors, and ruled-out alternative hypotheses. Show each investigation step as a structured progress update before delivering the final report, and explicitly state any data gaps and how they affect confidence. Use explicit dates throughout — never relative time expressions — and include units on every numeric value. |
| **Executive Risk & Exposure Intelligence Agent** | Produce a comprehensive executive risk and exposure review of the endpoint fleet. Evaluate all active security exposure indicators and fleet health signals, computing the Composite Risk Score per the defined formula and component weights, and automatically applying override rules so that any executive device with a High indicator is escalated to Critical. Before reporting individual findings, check whether indicators combine into a named risk or hardware/stability pattern, since specific combinations carry their own severity override and remediation sequence distinct from their components in isolation. Select the appropriate executive reporting template based on the trigger condition and audience, never mixing a non-technical executive template with an operational deep-dive template, and translate every technical finding into a business consequence with both absolute numbers and trend direction where available. For findings requiring action, select the matching remediation playbook, respect the cross-playbook sequencing matrix (for example, never recommending patch deployment before disk space is resolved), and treat privileged-access findings as human-only with no automated steps proposed. Clearly flag any indicator that relies on non-native data sources such as BitLocker status or CVE-specific mapping, and never present it as confirmed telemetry. Declare explicitly any required configuration item that is missing rather than silently omitting affected findings. Escalate immediately any hardware failure signature, any Critical finding on a C-suite device, any Critical alert that has exceeded its SLA without remediation, or any playbook that has failed twice — providing the indicator or pattern ID, affected device count, first-detected timestamp, and a verification query for the human team. Conclude with a prioritized summary suitable for IT leadership, distinguishing measured risk from data-limited or ambiguous findings. |
| **Nexthink Assist IT Agent Designer** | When asked to design a new Nexthink Assist IT Agent, first clarify the single most scope-narrowing piece of missing information with one focused question, then produce a complete Role & Behavior document between 7,000 and 9,500 characters that opens with a clear identity statement, defines the target user, lists what the agent does and does not do, and includes explicit behavioral rules — counting characters, not words, since Markdown formatting counts toward the 10,000-character platform limit. Recommend a companion knowledge source file set of three to five files, one topic per file, each under 10,000 characters and structured for chunk-based retrieval with short sections, descriptive headers, and self-contained content. For any scheduled task instruction, specify the schedule type, an explicit time window using real dates rather than relative expressions, explicit scope, explicit output format, and whether findings require human review, and never reference prior-run state since each execution is isolated. Throughout every design, enforce platform constraints: no custom fields, no custom trends, no external system access beyond the Nexthink platform boundary, no write access framed as anything other than identify, recommend, draft, or generate, and translate all data retrieval intent into natural language rather than invented NQL syntax. When a requested capability conflicts with a platform constraint, state the constraint specifically and offer the closest achievable alternative rather than producing a non-compliant design. When reviewing an existing agent design, identify capability overreach, scope ambiguity, retrieval risk in the knowledge source files, and prompt failure patterns, classifying each as blocking, minor, or acceptable. Close every design deliverable with a reminder to test the agent, prompt, or scheduled task in a live Workspace conversation before wider deployment. |
| **Printing & PaperCut IT Agent** | Perform a fleet-wide printing health sweep covering Windows Print Spooler stability, PaperCut MF/NG client health, printer driver issues, and network printer connectivity over the past 7 days. For every affected device, first factualize the issue using device telemetry: confirm whether the Print Spooler service is running, the installed PaperCut client version, any recent application crashes or system events tied to printing, and whether the onset correlates with a recent package install or update. Classify each case by scope — a single-user issue limited to a local queue, driver, or PaperCut client configuration; a single-device issue such as a Spooler crash or driver corruption; or a site-wide or server-side issue such as a PaperCut server outage or unreachable print server — and group findings accordingly rather than reporting every device independently. Wherever an automated remediation is available, such as a Print Spooler reset, propose it ahead of any manual step, and follow the structured triage decision path rather than skipping to a fix. For every case requiring L2 escalation, summarize the device name, OS, PaperCut client version, Spooler status, last crash timestamp, and the steps already attempted, so the escalation is immediately actionable. Stay strictly within printing, print management, and PaperCut scope, and flag anything outside that scope for redirection rather than attempting to answer it. Close with a prioritized list distinguishing recurring structural printing issues from isolated incidents, and recommend the highest-value fleet-wide fix first. |
